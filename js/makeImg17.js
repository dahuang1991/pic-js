...
$("#imgloader input").change(function(event){
    ...
    if(!oFile.type.match("jpeg") &&  !oFile.type.match("jpg") && !oFile.type.match("png")){
        event.target.files.length = 0;
        alert("只支持.jpeg/.jpg/.png类型的图片");
        $(".shadeBox").fadeOut();
        return false;
    }
    ...
    reader.onload = function(e){
        ...
        image.onload = function() {
            if(Orientation != "" && Orientation != 1 && Orientation != undefined) {
              ...
              $("#myImg").prepend("<img id='myPic' src='"+$(this).attr('src')+"'>");
            }
            $(".shadeBox").fadeOut();
            $(".imgContent").fadeOut();
...
function makeImg(){
    ...
    $("#myImg").append("" +
        "<div class='overf-h zImgInfo'>"+
        "<div class='left'>"+
        "<img class='urlimg' src='./img/urlimg.png'>"+
        "</div>"+
        "<img class='right zlogo' src='./img/zlogo.png'>"+
        "</div>"
    );
    window.setTimeout(function() {
        $(".shadeBox").fadeIn();
